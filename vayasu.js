...
document.getElementById('detectButton').addEventListener('click', function() {
    ...
    if (imageBase64) {
        detectAge(imageBase64);
    } else {
        alert('Please upload an image first.');
    }
});

async function detectAge(imageBase64) {
    ...
    try {
        ...
        document.getElementById('result').innerText = 'Error detecting age.';
    }
}
...
function displayResult(result) {
    ...
    if (result.faces && result.faces.length > 0) {
        const age = result.faces[0].attributes.age.value;
        resultDiv.innerHTML = `Detected age: ${age}`;
    } else {
        resultDiv.innerHTML = 'No faces detected or unable to determine age.';
    }
}
